...
const PANEL_BUTTON_MARGIN = 5;
const DESIGN_AREA_SIZE = GRID_SIZE * NUM_GRID_CELLS;
// The number of panel buttons that can fit in the height of the canvas
const PANEL_BUTTON_ROWS = Math.floor(DESIGN_AREA_SIZE / (GRID_SIZE + PANEL_BUTTON_MARGIN)); 
// The width the panel needs to be to fit all rows of buttons
const PANEL_WIDTH = Math.ceil(NUM_IMAGES / PANEL_BUTTON_ROWS) * (GRID_SIZE + PANEL_BUTTON_MARGIN) + PANEL_BUTTON_MARGIN;
let panelButtons = [];
...
function preload() {
    let spacing = GRID_SIZE + PANEL_BUTTON_MARGIN;
    let buttonsPerRow = (PANEL_WIDTH - PANEL_BUTTON_MARGIN) / spacing;
    // The x coordinate of the left most column of buttons
    let startX = DESIGN_AREA_SIZE + PANEL_BUTTON_MARGIN;
    for (let i = 0; i < NUM_IMAGES; i++) {
        let name = `${IMAGE_BASE}${i + 1}`;
        let img = loadImage(`assets/${name}.png`);
        let x = startX + (i % buttonsPerRow) * spacing;
        let y = PANEL_BUTTON_MARGIN + Math.floor(i / buttonsPerRow) * spacing;
        panelButtons.push(new PanelButton(name, img, x, y));
        
    }
}

function setup() {
    createCanvas(DESIGN_AREA_SIZE + PANEL_WIDTH, DESIGN_AREA_SIZE);
    save = createButton("Save");
    save.parent(select("main"));
    save.size(PANEL_WIDTH, 30);
    save.position(DESIGN_AREA_SIZE, height - 30);
...
function mouseClicked() {
    if (isMouseOverArea(0, 0, DESIGN_AREA_SIZE, DESIGN_AREA_SIZE) && selectedButton > -1) {
        const sprite = panelButtons[selectedButton].makeTile(getGridCoord(mouseX), getGridCoord(mouseY));
        placedSprites.push(sprite)
    } else if (isMouseOverArea(DESIGN_AREA_SIZE, 0, PANEL_WIDTH, DESIGN_AREA_SIZE)) {
        updatePanel();
    }
}
...
function saveLevel() {
    ...
    for (let sprite of placedSprites) {
        lines.push(`${sprite.getName()},${sprite.getX()},${sprite.getY()}`); 
    }
...
class PanelButton {
     ...
     * @param {string} name The name for the image in the button. Should be the same as the filename for the image without the extension.
    ...
    constructor(name, image, x, y) {
        this.#sprite = new Sprite(name, image, x, y);
    }
     ...
     */
    makeTile(x, y) {
        return new Sprite(this.#sprite.getName(), this.#sprite.getImage(), x, y);
    }
...
class Sprite {
     ...
     * @param {string} name The name to use for the sprite. Should be the file name of the image, minus the extension
     ...
     * @returns {string} The sprite's name
     */
    getName() {
        return this.#name;
    }
