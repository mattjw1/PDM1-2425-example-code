...
function draw() {
    drawRoom();
    drawPumpkin();
    drawGhost(ghost1X, ghost1Y, ghost1Scale, ghost1Alpha);
    drawGhost(ghost2X, ghost2Y, ghost2Scale, ghost2Alpha);
    drawGhost(ghost3X, ghost3Y, ghost3Scale, ghost3Alpha);
    drawSpider();
    updateGhosts();
    updateSpider();
}
 ...
 * 
 * NOTE: This code is very repetitive. Some of repetition could be made readable by creating 
 * more functions e.g. a function called randomX() that returns a random value between 200 and 601.
 * This would make the meaning of the code more clear but it would result in more code that doesn't 
 * seem worth it in my opinion. In coming weeks, we'll see a much better approach using classes.
 */
function createGhosts() {
    ...
    ghost1X = random(MIN_GHOST_X, MAX_GHOST_X);
    ghost1Y = random(MIN_GHOST_Y, MAX_GHOST_Y);
    ghost1Scale = random(MAX_SCALE);
    ghost1Alpha = random(MAX_ALPHA) * 255;

    ghost2X = random(MIN_GHOST_X, MAX_GHOST_X);
    ghost2Y = random(MIN_GHOST_Y, MAX_GHOST_Y);
    ghost2Scale = random(MAX_SCALE);
    ghost2Alpha = random(MAX_ALPHA) * 255;

    ghost3X = random(MIN_GHOST_X, MAX_GHOST_X);
    ghost3Y = random(MIN_GHOST_Y, MAX_GHOST_Y);
    ghost3Scale = random(MAX_SCALE);
    ghost3Alpha = random(MAX_ALPHA) * 255;
}

/**
 * Updates the global variables controlling the ghosts.
 * 
 * NOTE: As with createGhosts(), this code is very repetitive but it is left as is 
 * because breaking it down further will not make much difference. Classes will also 
 * improve this.
 */
function updateGhosts() {
    const SCALE_INC = 0.001;

    ghost1X += randomMovement();
    ghost1Y += randomMovement();
    ghost1Scale += SCALE_INC;
    ghost1Alpha = ghost1Scale * 255;

    ghost2X += randomMovement();
    ghost2Y += randomMovement();
    ghost2Scale += SCALE_INC;
    ghost2Alpha = ghost2Scale * 255;

    ghost3X += randomMovement();
    ghost3Y += randomMovement();
    ghost3Scale += SCALE_INC;
 ...
 * 
 * NOTE: This is an example of how createGhosts() / updateGhosts() could be broken down 
 * further. It only contains one line of code but it does make the purpose of that code 
 * a little clearer. Is it worth it? That is subjective.
